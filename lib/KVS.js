...
class KVS {
    ...
    _add_watcher (key, opts, callback) {
        ...
        if (kv) {
            if (kv.data.value !== undefined && !opts.noInitial) {
                const value = kv.data.value;
                const revision = kv.data.revision;
        ...
        kv.watchers.add(opts.id, key, callback);
        ...
        return () => {
            ...
            this._log('debug', 'Watcher removed', { key, watcher_id: opts.id, watchers: kv.watchers.size });
    ...
    _new_key (key, value) {
        ...
        function notify (watchers) {
            setImmediate(function () {
                watchers.notify({ key, value, revision: 0 });
            });
        }
        ...
        for (const [range, watchers] of this._range_watchers) {
            if (key.indexOf(range) === 0) {
                ...
                for (const callback of watchers) {
                    kv.watchers.add(callback._id, callback);
                }
    ...
    delete (key) {
        ...
        if (kv) {
            ...
            if (kv.watchers && kv.watchers.size) {
    ...
    set (key, value) {
        ...
        if (!kv) {
            ...
            if (updated) {
                ...
                setImmediate(function () {
                    if (kv.watchers) {
                        kv.watchers.notify({ key, value, revision });
                    }
                });
            }
        }
