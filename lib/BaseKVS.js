    ...
    */
    ...
    set (key, value) {
        ...
        if (!kv) {
            ...
            this._log('debug', 'Record created', { key, revision: kv.data.revision });
            this._log('trace', 'Record data', { key, data: value, revision: kv.data.revision });

        } else if (kv.set(value)) {
            updated = 2;
            this._log('debug', 'Record updated', { key, revision: kv.data.revision });
            this._log('trace', 'Record data', { key, data: value, revision: kv.data.revision });
        }
