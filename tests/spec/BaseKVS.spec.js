...
const KVS = require('../../lib/BaseKVS.js');

describe('lib/BaseKVS.js', function () {
    const logs = [];
    ...
    it('.set() adds a new key and returns 1', function () {
        expect(kvs.set('foo', 123)).to.equal(1);
    ...
    it('.set() updates existing key value and returns 2', function () {
        expect(kvs.set('foo', 456)).to.equal(2);
        expect(kvs.get('foo').key).to.equal('foo');
    ...
    it('.set() returns 0 if value has not changed', function () {
        expect(kvs.set('foo', 456)).to.equal(0);
        expect(kvs.get('foo').key).to.equal('foo');
        expect(kvs.get('foo').revision).to.equal(1);
        expect(kvs.get('foo').value).to.equal(456);
    });
    ...
    it('.find() returns matching keys', function () {
        expect(kvs.set('bar', 123)).to.equal(1);
        expect(kvs.set('baz', 123)).to.equal(1);
